...
async function sendMulticast(tokens, title, body) {
    ...
    for (let i = 0; i < tokens.length; i += FCM_BATCH_SIZE) {
        ...
        try {
            ...
            console.log(`[FCM] batch ${Math.floor(i / FCM_BATCH_SIZE) + 1}: ` +
 ...
 */
exports.onReligionPointsUpdated = (0, firestore_1.onDocumentUpdated)('aggregates/religionPoints', async (event) => {
    ...
    // 1위 변동 없으면 알림 불필요
    if (!topAfter || topBefore === topAfter)
        return;
    ...
    const etcTokens = allTokens
        ...
        .map((t) => t.token);
    await Promise.all([
        sendMulticast(koTokens, '🏆 순위 변동!', `${(0, religion_names_1.getReligionName)(topAfter, 'ko')}이(가) 1위로 올라섰습니다!`),
        sendMulticast(enTokens, '🏆 Ranking Update!', `${(0, religion_names_1.getReligionName)(topAfter, 'en')} is now #1!`),
        sendMulticast(zhTokens, '🏆 排名更新！', `${(0, religion_names_1.getReligionName)(topAfter, 'zh')}荣登第一！`),
        // 기타 언어는 영어 메시지
        sendMulticast(etcTokens, '🏆 Ranking Update!', `${(0, religion_names_1.getReligionName)(topAfter, 'en')} is now #1!`),
    ]);
});
/**
 ...
 */
exports.sendDailyRankingSummary = (0, scheduler_1.onSchedule)({ schedule: '0 9 * * *', timeZone: 'Asia/Seoul' }, async () => {
    const db = (0, firestore_2.getFirestore)();
    ...
    const rankBadge = ['🥇', '🥈', '🥉'];
    const bodyKo = top3
        .map((r, i) => `${rankBadge[i]} ${(0, religion_names_1.getReligionName)(r.id, 'ko')}: ${r.pts.toLocaleString()}P`)
        .join(' · ');
    const bodyEn = top3
        .map((r, i) => `${rankBadge[i]} ${(0, religion_names_1.getReligionName)(r.id, 'en')}: ${r.pts.toLocaleString()}P`)
        .join(' · ');
    const bodyZh = top3
        .map((r, i) => `${rankBadge[i]} ${(0, religion_names_1.getReligionName)(r.id, 'zh')}: ${r.pts.toLocaleString()}P`)
        .join(' · ');
    await Promise.all([
...
//# sourceMappingURL=ranking_notifications.js.map